...
pushd $FAST_DDS
	...
	mkdir ../build
	cd ../build
	cmake .. -DCMAKE_INSTALL_PREFIX=$FASTDDS_PROJ/install -DCMAKE_BUILD_TYPE=$BUILD_TYPE -DEPROSIMA_BUILD_TESTS=OFF -DBUILD_SHARED_LIBS=OFF -DEPROSIMA_BUILD=ON -DSECURITY=ON -DSHM_TRANSPORT_DEFAULT=OFF -DFASTDDS_STATISTICS=OFF -DCMAKE_CXX_FLAGS="-Wno-enum-constexpr-conversion"
	cmake --build . --target install
popd
